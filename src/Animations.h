...
private:
  ...
  bool draw(float dt) {
    ...
    for(uint8_t i=0;i<thickness;i++) {
      x1 = sinf(phase + mapf(y, 0, display.height-1, 0, 2*PI) + i*PI/60);
      z1 = cosf(phase + mapf(y, 0, display.height-1, 0, 2*PI) + i*PI/60);
      x2 = mapf(x1, -1.1f, 0.9f, 0, display.width-1);
      z2 = mapf(z1, -1.1f, 0.9f, 0, display.depth-1);
      Color c = Color((wheel>>8) + x2*10 + y*32 + z2*10, &RainbowGradientPalette[0]);
      display.color(x2, y, z2) = c;
    }
    if(timer1.update()) {
      uint8_t state = 0;
      if(stage == state++)
        top < display.height ? top++ : stage++;
      if(stage == state++)
        thickness < 20 ? thickness++ : stage++;
      if(stage == state++)
        if(timer2.update()) stage++;
      if(stage == state++)
        bottom < display.height ? bottom++ : stage++;
      if(stage == state++)
        return true;
    }
    return false;
...
private:
  ...
  bool draw(float dt) {
    ...
    if(phase/(2*PI) >= 1)
      if(--display.globalBrightness == 0xff)
        return true;

    for(uint8_t x=0;x<display.width;x++)
    for(uint8_t y=0;y<display.height;y++)
    for(uint8_t z=0;z<display.depth;z++) {
      Color c = Color((wheel>>8) + x*x1+y*y1+z*z1, &RainbowGradientPalette[0]);
...
private:
  ...
  bool draw(float dt) {
    ...
    if(timer1.update()) {
      down();
      for(uint8_t d=random(0,2);d>0;d--) {
...
private:
  void init() {
    position = Vector3(display.width/2.0f, display.height/2.0f, display.depth/2.0f);
    velocity = Vector3(generator.nextRandom(4.0f,10.0f),
...
private:
  void init() {
    ...
    source = Vector3(
      generator.nextGaussian((float)display.width/2.0f, 0.40f), 0,
  ...
  bool draw(float dt) {
    ...
    if(target.y > 0) {
      ...
      // If missile falls back to earth or moved past the target explode the missile
      if ((temp.y > missile.position.y) | (missile.position.y > target.y))  {
        ...
        numDebris = random(5, sizeof(debris)/sizeof(Object2));
        ...
        for(int i=0;i<numDebris;i++) {
          ...
          debris[i] = { temp, explode, uint8_t(hue + random(0,64)),
            1.0f, generator.nextRandom(1.0f, 3.0f)};
        }
      } else if(missile.position.inside( Vector3(0,0,0),
          Vector3(display.width, display.height, display.depth)))
            display.color(missile.position.x, missile.position.y, missile.position.z) = Color(Color::WHITE);
    }
    ...
    if(target.y==0) {
      ...
      for(uint8_t i=0;i<numDebris;i++) {
        ...
        if(debris[i].position.inside( Vector3(0,0,0),
          Vector3(display.width, display.height, display.depth)))
            display.color(debris[i].position.x, debris[i].position.y, debris[i].position.z) += c;
      }
      if(visible==0) {
        if(--loops==0)
          return true;
...
private:
  void init() {
    ...
    timer1 = generator.nextRandom(0.05f, 0.20f);
  ...
  bool draw(float dt) {
    ...
    if(timer1.update()) {
      display.color(random(display.width),random(display.height),random(display.depth)) = Color(0,255);
    }
    if(timer2.update()) {
      if(--loops==0)
        return true;
