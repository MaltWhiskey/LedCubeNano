...
Color::Color(const uint8_t index, const uint8_t* pallete) {
  ...
  if(pallete[i]!=index) {
    red   = map8( index, pallete[i-4], pallete[i], pallete[i+1-4], pallete[i+1] );
    green = map8( index, pallete[i-4], pallete[i], pallete[i+2-4], pallete[i+2] );
    blue  = map8( index, pallete[i-4], pallete[i], pallete[i+3-4], pallete[i+3] );
  }
  else {
    red   = pallete[i+1];
    green = pallete[i+2];
...
// Create a color blend between source and target. Scaler determine how much between
// scaler = 000 -> (source +   0% delta) = source
// scaler = 255 -> (source + 100% delta) = target 
...
Color& Color::operator+(const Color& c_) const {
  Color c = *this;
  return (c+=c_);
}
Color& Color::operator-(const Color& c_) const {
